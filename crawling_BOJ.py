...
def savewithtext(
    ...
    with open("samsung_1.tsv", mode="w", newline='') as f:
        wr = csv.writer(f, delimiter='\t')
        for i in range(len(problem_num)):
            problem_info = [problem_num[i], problem_sub[i], " ".join(problem_des[i].split()), pb_info_list[i][-5], pb_info_list[i][-3], pb_info_list[i][-1]]
...
if whole_response.status_code == 200:
    ...
    for pb in pbs:
        ...
        pb_info = [info.text for info in pb.find_all()]
